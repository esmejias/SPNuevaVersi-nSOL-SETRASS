CREATE             PROCEDURE dbo.reformasdeestatutos_controlarDocumentos
  @CODIGO BIGINT, 
  @TIPO_TRAMITE INT, 
  @ESTADO TINYINT, 
  @FASE INT,
...
BEGIN
  ...
  /* Autora: Dayana Fernández
     Trámite: APROBACIÓN DE REFORMA DE ESTATUTOS DE LAS ORGANIZACIONES SOCIALES
...
*/
  ...
  -------------------------------------------------------------------------
  -- Moldes especiales (actuante)
  -------------------------------------------------------------------------
  DECLARE @CARNET_COLEGIACION INT = 2189;
  DECLARE @CARTA_PODER        INT = 21888;
  ...
  BEGIN
    ...
    FROM dbo.datos_adicionales_11 WITH (NOLOCK)
 ...
 IF @EsSol = 1
BEGIN
  -- Moldes visibles PARA CIUDADANO según tabla
  ...
  -- Base sin especiales (nunca aparecen por defecto)
  INSERT INTO #ALLOW(codigo_molde, src)
  SELECT codigo_molde, 'C'
  FROM #CAND_SOL
  WHERE codigo_molde NOT IN (2188, 2189);

  -- Especiales solo si aplica la calidad (y solo si la tabla los permite)
  IF @CALIDAD_ACTUANTE IN (N'Apoderado Legal', N'Representante Legal')
  BEGIN
    INSERT INTO #ALLOW(codigo_molde, src)
    SELECT codigo_molde, 'A'
    FROM #CAND_SOL
    WHERE codigo_molde IN (2188, 2189);
  END
END
ELSE
BEGIN
  -- Funcionario: NO filtrar por visibilidad
  INSERT INTO #ALLOW(codigo_molde, src)
  SELECT f.codigo_molde, 'C'
  FROM #CFG_APLICA f
  LEFT JOIN #DT dt ON dt.codigo_molde_documento = f.codigo_molde
  WHERE NOT (
    f.codigo_molde IN (2188, 2189)
...
WHERE dt.codigo_tramite = @CODIGO
  ...
  BEGIN
    UPDATE D
      SET D.visibilidad = 1,
          D.editable    = CASE
                            WHEN D.codigo_molde_documento IN (2188, 2189) THEN 0
                            WHEN F.codigo_obligatoriedad = 12 THEN 0
    ...
    UPDATE #DOC
    SET editable = 0
    WHERE codigo_molde_documento IN (2188, 2189);
  END
