CREATE  PROCEDURE dbo.comunicacioncambiojuntadirectiva_controlarDocumentos
...
BEGIN
  ...
  Trámite: COMUNICACIÓN DE CAMBIO DE JUNTA DIRECTIVA
...
*/
  ...
  BEGIN
    ...
    IF @TIPO_TRAMITE = 13 AND @CALIDAD_ACTUANTE IS NOT NULL
    BEGIN
      INSERT INTO #ALLOW(codigo_molde, src)
      SELECT codigo_molde, 'C'
      FROM #CAND_SOL
      WHERE codigo_molde NOT IN ( 2190 , 2191 );

      IF (@CALIDAD_ACTUANTE IN (N'Apoderado Legal', N'Representante Legal'))
        INSERT INTO #ALLOW(codigo_molde, src) SELECT 2190, 'A'
        UNION ALL SELECT 2191, 'A';
...
WHERE dt.codigo_tramite = @CODIGO
    ...
    UPDATE D
      SET D.visibilidad = 0,
          D.editable    = CASE 
                            ...
                            ELSE COALESCE(F.editable_solicitante, CASE WHEN @EnPrep=1 THEN 1 ELSE 0 END)
                          END,
          D.obligatorio = CASE 
  ...
  BEGIN
    UPDATE D
      SET D.visibilidad = 1,
          D.editable    = CASE
                            WHEN D.codigo_molde_documento IN (2190 , 2191) THEN 0
                            WHEN F.codigo_obligatoriedad = 12 THEN 0
    ...
    UPDATE #DOC
    SET editable = 0
    WHERE codigo_molde_documento IN (2190 , 2191);
  END
