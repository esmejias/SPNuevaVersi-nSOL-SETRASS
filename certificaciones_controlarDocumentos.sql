CREATE           PROCEDURE dbo.certificaciones_controlarDocumentos
  @CODIGO BIGINT, 
  @TIPO_TRAMITE INT, 
  @ESTADO TINYINT, 
  @FASE INT,
...
BEGIN
  ...
  Trámite: CERTIFICACIONES Y CONSTANCIAS EMITIDAS POR LA DIRECCIÓN GENERAL DEL TRABAJO (DGT)
...
*/
  ...
  -------------------------------------------------------------------------
  -- Moldes especiales (actuante, TT=3)
  -------------------------------------------------------------------------
 DECLARE @CARNET_COLEGIACION INT = 40;
 DECLARE @CARTA_PODER        INT = 39;
 DECLARE @DNI                INT = 41;
 DECLARE @CERTIF_SINDICATO   INT = 42;

  -------------------------------------------------------------------------
  -- Calidad de actuante (solo TT=3)
  -------------------------------------------------------------------------
  DECLARE @CALIDAD_ACTUANTE NVARCHAR(4000) = NULL;
  IF @TIPO_TRAMITE = 3
  BEGIN
    SELECT @CALIDAD_ACTUANTE = LTRIM(RTRIM(CAST(calidad_actuante AS NVARCHAR(4000))))
    FROM dbo.datos_adicionales_3 WITH (NOLOCK)
  ...
  BEGIN
    ...
    IF @TIPO_TRAMITE = 3 AND @CALIDAD_ACTUANTE IS NOT NULL
    BEGIN
      INSERT INTO #ALLOW(codigo_molde, src)
      SELECT codigo_molde, 'C'
      FROM #CAND_SOL
      WHERE codigo_molde NOT IN (41, 42, 40, 39 );
    
      IF (@CALIDAD_ACTUANTE = N'Ciudadano/a')
        INSERT INTO #ALLOW(codigo_molde, src) SELECT @DNI, 'A';

      IF (@CALIDAD_ACTUANTE = N'Apoderado/a Legal')
        INSERT INTO #ALLOW(codigo_molde, src) SELECT @CARNET_COLEGIACION, 'A'
        UNION ALL SELECT @CARTA_PODER, 'A';

      IF (@CALIDAD_ACTUANTE = N'Miembro del Sindicato')
        INSERT INTO #ALLOW(codigo_molde, src) SELECT @CERTIF_SINDICATO, 'A';
    END
    ELSE
    BEGIN
      INSERT INTO #ALLOW(codigo_molde, src)
  ...
  BEGIN
    ...
    WHERE NOT (
      f.codigo_molde IN (@CARNET_COLEGIACION, @CARTA_PODER, @DNI, @CERTIF_SINDICATO)
      AND dt.codigo_molde_documento IS NULL
    );
  END

  -----------------------------------------------------------------------
-- FILTRO: Solo permitir el molde T* correspondiente al tipo de trámite
-----------------------------------------------------------------------

-- Declaración de códigos T1–T5
DECLARE @T1 INT = 34;
DECLARE @T2 INT = 35;
DECLARE @T3 INT = 36;
DECLARE @T4 INT = 37;
...
DECLARE @Molde_TT INT = NULL;

IF @TIPO_TRAMITE = 3 -- Solo aplica para Certificaciones y Constancias
BEGIN
    DECLARE @tipo NVARCHAR(255);

    SELECT @tipo = tipo_tramite
    FROM dbo.datos_adicionales_3 WITH (NOLOCK)
    WHERE codigo_tramite = @CODIGO;

    IF @tipo LIKE 'T1%' SET @Molde_TT = @T1;
    IF @tipo LIKE 'T2%' SET @Molde_TT = @T2;
    IF @tipo LIKE 'T3%' SET @Molde_TT = @T3;
    IF @tipo LIKE 'T4%' SET @Molde_TT = @T4;
    IF @tipo LIKE 'T5%' SET @Molde_TT = @T5;

    -- Eliminar todos los moldes T1–T5 excepto el correspondiente
    DELETE FROM #ALLOW
    WHERE codigo_molde IN (@T1, @T2, @T3, @T4, @T5)
      AND codigo_molde <> @Molde_TT;
END
...
---------------------------------------------------------------------------
-- 4B) Para el ciudadano: ocultar documentos de gestión interna (T1–T5)
--     mientras no exista un archivo real cargado
---------------------------------------------------------------------------
IF @EsSol = 1
BEGIN
    DELETE FROM #DOC
    WHERE codigo_molde_documento IN (@T1, @T2, @T3, @T4, @T5) -- ya declaradas arriba
      AND (codigo_documento IS NULL
           OR nombre_archivo IS NULL
           OR nombre_archivo = '__STUB__.pdf');
END
  ...
  IF @EsSol = 1
  BEGIN
    -- 1. Bloqueo preventivo: Si ya existe un documento físico, no es editable.
    ...
    UPDATE D
      SET D.visibilidad = 0,
          ...
          D.editable    = CASE 
                            WHEN @EnPrep = 0 THEN 0 -- Si no está en preparación, NADA es editable
  ...
  ELSE
  BEGIN
    -- Lógica para el Responsable (se mantiene igual o similar)
    UPDATE D
      SET D.visibilidad = 1,
          D.editable    = CASE
                            WHEN D.codigo_molde_documento IN (@CARNET_COLEGIACION, @CARTA_PODER, @DNI, @CERTIF_SINDICATO) THEN 0
                            WHEN F.codigo_obligatoriedad = 12 THEN 0
    ...
    UPDATE #DOC
    SET editable = 0
    WHERE codigo_molde_documento IN (@CARNET_COLEGIACION, @CARTA_PODER, @DNI, @CERTIF_SINDICATO);
  END
